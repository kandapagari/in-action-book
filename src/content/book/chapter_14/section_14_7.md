...
target_words: 2000
status: draft
prereqs: §14.1–§14.6; why reasoning and reacting want clocks an order of magnitude apart and one forward pass can only serve one of them, Helix's thin continuous latent between a 7B System 2 and an 80M System 1, GR00T N1's wide token channel and diffusion action head plus its N1.5–N1.7 data lineage, Gemini Robotics-ER as an embodied-reasoning third family and Gemini Robotics 2 as a fourth that folds locomotion into one whole-body policy, the worst-case-jitter latency budget that decides whether a stack ships, and the deployment record that shows the fast half is where a shift lives or dies
key_refs:
  ...
  - Google DeepMind (2026). Gemini Robotics 2 and the Embodied Reasoning family. deepmind.google/blog; arXiv:2510.03342 (ER 1.5).
---
...
more than an order of magnitude apart. Chapter 14 spent six sections on the
...
that decides whether the split works, §14.5 checked the whole idea against
robots that have run for real hours on real floors, and §14.6 added Gemini
Robotics 2 as a fourth family that pushes the fast half all the way down to the
legs. The chapter's job was to make the two-clock structure concrete enough that
Part 5's fine-tuning and safety questions read as questions about a specific
machine rather than about VLAs in the abstract.
...
*The reasoning-first family kept growing, and by the end of the chapter it had
gone whole-body.* Google DeepMind's Gemini Robotics-ER (arXiv:2510.03342) keeps
the two-clock split but loads the reasoning half harder. Its "Embodied Thinking"
pattern has System 2 reason through spatial and temporal steps, emit intermediate
points and trajectories on the image, and hand a plan down rather than a bare
intent, which is the embodied chain-of-thought idea §18.4 develops in full. Its
Motion Transfer carries a skill learned on one embodiment to another through the
shared reasoning layer, a direct attack on the cross-embodiment problem §18.1
takes up. §14.6 then showed where that line went next: Gemini Robotics 2 (July
2026) keeps the slow-reasoner, fast-actor skeleton but widens the fast half to
drive the entire body, legs included, under one policy, erasing the locomotion-
versus-manipulation seam every earlier stack still had. So the chapter's family
tree has four branches that agree on the skeleton and disagree about what each
clock should own: Helix bets thin latent and closed end-to-end training, GR00T
bets wide token channel and open video-heavy data, Gemini Robotics-ER bets on a
reasoner that thinks explicitly before the fast half moves, and Gemini Robotics 2
bets on a fast half that owns balance and locomotion too.
...
why flow-matching heads that need only a handful of steps have an edge here. A
whole-body policy like Gemini Robotics 2 only tightens this, because now the legs
ride the fast clock too.
...
below System 1, explain why absorbing locomotion into the learned stack matters
for §17.5's certification problem, and see why Gemini Robotics 2 taking that step
further, into one whole-body policy, is the same trend reaching its limit.
...
repetition, that a 92% light-bulb demo is the same warning in newer clothes, that
the failures which show up over hours are jitter and contact-timing failures
rather than reasoning failures, and that the teams whose claims aged well reported
repetition counts and shift lengths and admitted where dexterity still falls short
of a human hand.
...
embodiment, and Gemini Robotics On-Device 2's few-hours re-targeting from §14.6 is
the same idea pushed toward the practitioner.

The safety thread runs straight into Chapter 17. Helix-02's System 0 made the
balance layer as opaque as the manipulation layer, and Gemini Robotics 2 put that
balance inside the manipulation policy itself, so you can no longer read the
...
tracking loop, and Helix-02's System 0 and Gemini Robotics 2's whole-body policy
push that rediscovery down to the metal by learning the locomotion that used to be
derived from a dynamics model. §18.1 is where the book returns to what that
dissolving boundary means for the field.
...
single-system VLA cannot give it. Four families, Helix, GR00T, Gemini Robotics-ER,
and Gemini Robotics 2, agree on that skeleton and disagree productively about its
details, with Gemini Robotics 2 extending the fast half all the way to unified
whole-body control. The design earns its complexity only on tasks fast or dynamic
enough to need it, its latency budget is decided by worst-case jitter rather than
average throughput, and its deployment record says the fast half run for real
hours is where success is actually won or lost. With the two-clock structure
understood at this depth, Chapter 15 can ask what data trains these systems and
how anyone is supposed to measure whether they work.
