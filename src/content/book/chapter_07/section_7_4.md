...
any policy, because its Bellman target $r + \gamma \max_{a'} Q(s', a')$
depends only on the transition, not on who chose the action. But that
$\max_{a'}$ is an optimization over the action space, and in a
...
The trick is to replace the intractable $\max_{a'} Q(s', a')$ with a
*learned* maximizer: train a deterministic actor network
$\mu_\phi(s)$ whose job is to output the action that maximizes the
...
The critic $Q_\theta$ is trained by ordinary temporal-difference
...
gradient of the critic with respect to the action, $\nabla_a Q$, says
"to raise the Q-value, nudge the action this way"; the actor Jacobian
$\nabla_\phi \mu$ then translates that desired action-nudge into a
weight update. Because both terms are computed on states $s$ sampled
from the replay buffer $\mathcal{D}$ — states collected by *old* policies
...
transition be reused many times, and slowly-updated *target networks*
$Q_{\theta'}$ and $\mu_{\phi'}$ that supply the bootstrap target $y$ so
...
$\theta' \leftarrow \tau\theta + (1-\tau)\theta'$ with $\tau \approx
0.005$, rather than DQN's periodic hard copy.
...
collection time — $a = \mu_\phi(s) + \mathcal{N}$ — and storing the
...
feed straight into the next Bellman target through $Q(s', \mu(s'))$, get
...
$Q_{\theta_1}, Q_{\theta_2}$ and form the Bellman target using the
*minimum* of the two:
...
The second is *target policy smoothing*. The action $\tilde{a}$ in the
target above is not $\mu_{\phi'}(s')$ exactly but that action plus a
small clipped noise: $\tilde{a} = \mu_{\phi'}(s') +
\operatorname{clip}(\epsilon, -c, c)$. The reasoning is that a good action
...
The temperature $\alpha$ sets the exchange rate between reward and
...
$a = \tanh(\mu_\phi(s) + \sigma_\phi(s)\odot\xi)$ with $\xi \sim
\mathcal{N}(0,I)$ — so the gradient of the expected (Q minus
...
came shortly after: rather than tune $\alpha$ by hand, make it *automatic*
