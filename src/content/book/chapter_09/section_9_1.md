...
$\hat{P}_\theta$ and $\hat{R}_\theta$ — fit by watching transitions
$(s_t, a_t, s_{t+1}, r_t)$ roll past and minimizing prediction error.
...
a manipulator as $\ddot{q} = M(q)^{-1}(\tau - C(q,\dot q)\dot q - g(q))$
...
action, $\pi(a \mid s)$ — "what should I do?" A *world model* is a
function from state and action to next state, $\hat{P}(s' \mid s, a)$ —
...
compresses each 64×64 frame into a small latent vector $z_t$ — call it
roughly 30 numbers instead of roughly 12,000 pixels. A **memory
model**, a recurrent network with a mixture-density output head,
predicts the next latent: $\hat{P}(z_{t+1} \mid z_t, a_t, h_t)$, where
$h_t$ is the RNN's hidden state carrying the history. This is the world
...
$(z_t, h_t)$ to an action, is the policy.
